        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <SortableHeader columnKey="shipment_number" sortKey={sortKey} sortDir={sortDir} onToggle={toggleSort} className="px-4 py-2.5 text-xs font-semibold text-slate-500">No. Shipment</SortableHeader>
              <SortableHeader columnKey="vendor_name" sortKey={sortKey} sortDir={sortDir} onToggle={toggleSort} className="px-4 py-2.5 text-xs font-semibold text-slate-500">Vendor</SortableHeader>
              <SortableHeader columnKey="shipment_date" sortKey={sortKey} sortDir={sortDir} onToggle={toggleSort} className="px-4 py-2.5 text-xs font-semibold text-slate-500">Tanggal</SortableHeader>
              <th className="text-left px-4 py-2.5 text-xs font-semibold text-slate-500">Items</th>
              <SortableHeader columnKey="status" sortKey={sortKey} sortDir={sortDir} onToggle={toggleSort} className="px-4 py-2.5 text-xs font-semibold text-slate-500">Status</SortableHeader>
              <SortableHeader columnKey="inspection_status" sortKey={sortKey} sortDir={sortDir} onToggle={toggleSort} className="px-4 py-2.5 text-xs font-semibold text-slate-500">Inspeksi</SortableHeader>
                      ...
                      <div key={pi.id} className="flex items-center justify-between px-3 py-2 hover:bg-blue-50 cursor-pointer transition-colors" onClick={() => addShipmentItem(pi)}>
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-[11px] text-slate-400 mt-0.5">
                            ...
                            {alreadySent > 0 && <span className="ml-1 text-slate-400">(terkirim: {alreadySent})</span>}
                          </div>
                        </div>
                      </div>
                    );
                      ...
                      <div key={idx} className={`flex items-center justify-between rounded-lg px-3 py-2 ${over ? 'bg-red-50 border border-red-200' : 'bg-blue-50'}`}>
                        ...
                        <div className="flex items-center gap-2">
                          <input type="number" min="1" max={maxQty || undefined} value={item.qty_sent}
                            ...
                            className={`w-20 border rounded px-2 py-1 text-xs text-right ${over ? 'border-red-400' : 'border-slate-200'}`} />
